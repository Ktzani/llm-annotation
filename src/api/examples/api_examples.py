...
def wait_for_anotacao(annotation_id: str, check_interval: int = 5, timeout: int = 3600):
    """
    Aguarda a conclusão de uma anotação
    ...
    while time.time() - start_time < timeout:
        response = requests.get(f"{API_BASE_URL}/annotation/{annotation_id}")
...
def exemplo_basico():
    ...
    print("\nCriando anotação...")
    response = requests.post(f"{API_BASE_URL}/annotation", json=config)
    
    if response.status_code == 200:
        data = response.json()
        annotation_id = data["annotation_id"]
        print(f"Anotação criada: {annotation_id}")
        
        # Aguardar conclusão
        result = wait_for_anotacao(annotation_id)
...
def exemplo_avancado():
    ...
    if response.status_code == 200:
        ...
        result = wait_for_anotacao(annotation_id, check_interval=10)
...
# =============================================================================
# EXEMPLO 4: LISTAR E MONITORAR ANOTAÇÕES
# =============================================================================

def exemplo_listar_anotacoes():
    """Lista todas as anotações"""
    print("\n" + "=" * 70)
    print("EXEMPLO 4: LISTAR ANOTAÇÕES")
    print("=" * 70)
    
    response = requests.get(f"{API_BASE_URL}/annotation")
    
    if response.status_code == 200:
        data = response.json()
        print(f"\nTotal de anotações: {data['total']}\n")
        
        for exp in data['annotations']:
            print(f"ID: {exp['annotation_id']}")
...
def exemplo_comparacao_prompts():
    ...
    # Criar anotações para cada tipo de prompt
    for prompt_type in prompt_types:
        ...
        print(f"\nCriando anotação com prompt: {prompt_type}")
        response = requests.post(f"{API_BASE_URL}/annotation", json=config)
        
        if response.status_code == 200:
            data = response.json()
            annotation_ids.append((prompt_type, data["annotation_id"]))
            print(f"  ✓ Anotação criada: {data['annotation_id']}")
        else:
            print(f"  ✗ Erro: {response.status_code}")
    
    # Aguardar todas as anotações
    print("\n" + "=" * 70)
    print("AGUARDANDO CONCLUSÃO DAS ANOTAÇÕES...")
    print("=" * 70)
    
    results = {}
    for prompt_type, exp_id in annotation_ids:
        print(f"\nAguardando anotação {prompt_type}...")
        result = wait_for_anotacao(exp_id, check_interval=5)
