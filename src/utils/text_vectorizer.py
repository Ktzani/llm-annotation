...
class TextVectorizer:
    ...
    def fit(self, texts) -> "TextVectorizer":
        """Aprende o vocabulário/IDF apenas nos textos de treino (fit part)."""
        logger.info(f"Ajustando TF-IDF em {len(texts)} textos (fit): {self._params}")
        self.vectorizer.fit(texts)
        return self

    def transform(self, texts) -> csr_matrix:
        """
        Vetoriza textos usando o vocabulário já aprendido em ``fit`` — sem
        reaprender. Essencial para evitar vazamento ao vetorizar o held-out.
        """
